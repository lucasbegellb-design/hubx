...
const ENTETES_PAIEMENTS = ["Réf.", "Fournisseur", "N° PO", "Désignation", "Type", "Montant", "Devise", "Échéance", "Payé le", "Statut"];
const paiements: Ligne[] = [
  ["PAY-101", "Shenzhen Wingtech Co.", "PO-2026-031", "Ailes Swift (lot 1)", "Acompte 30 %", 9600, "USD", d("2026-08-05"), d("2026-08-04"), "Payé"],
  ["PAY-102", "Shenzhen Wingtech Co.", "PO-2026-031", "Ailes Swift (lot 1)", "Solde 70 %", 22400, "USD", d("2026-09-28"), null, "À payer"],
  ["PAY-103", "Dongguan Precision Motors", "PO-2026-034", "Micro-moteurs BB-M3", "Acompte 30 %", 5850, "USD", d("2026-08-20"), d("2026-08-21"), "Payé"],
  ["PAY-104", "Dongguan Precision Motors", "PO-2026-034", "Micro-moteurs BB-M3", "Solde 70 %", 13650, "USD", d("2026-10-15"), null, "À payer"],
  ["PAY-105", "Ningbo Packaging Ltd", "PO-2026-036", "Coffrets cadeau Noël", "Totalité", 48000, "CNY", d("2026-09-22"), null, "En attente"],
  ["PAY-106", "Guangzhou Battery Tech", "PO-2026-038", "Batteries LiPo 3,7 V", "Acompte 50 %", 3200, "USD", d("2026-09-10"), d("2026-09-12"), "Payé"],
  ["PAY-107", "Guangzhou Battery Tech", "PO-2026-038", "Batteries LiPo 3,7 V", "Solde 50 %", 3200, "USD", d("2026-10-01"), null, "À payer"],
  ["PAY-108", "Xiamen Mold Factory", "PO-2026-040", "Moule coque Swift v2", "Totalité", 12500, "EUR", d("2026-11-05"), null, "À payer"],
  ["PAY-109", "Shenzhen Wingtech Co.", "PO-2026-042", "Ailes Swift (lot 2)", "Acompte 30 %", 9600, "USD", d("2026-09-30"), null, "À payer"],
];

const ENTETES_LIVRAISONS = ["N° PO", "Fournisseur", "Transporteur", "Départ usine prévu", "Livraison prévue", "Livraison réelle", "Statut"];
...
function classeur(pai: Ligne[], liv: Ligne[]) {
  ...
  const f1 = XLSX.utils.aoa_to_sheet([["Suivi fournisseurs Chine — paiements 2026"], [], ENTETES_PAIEMENTS, ...pai], { cellDates: true });
...
function versDonnees(wb: XLSX.WorkBook) {
  ...
  return normaliserClasseur(
    relu.SheetNames.map((nom) => ({ nom, lignes: XLSX.utils.sheet_to_json<unknown[]>(relu.Sheets[nom], { header: 1, raw: true, defval: null }) })),
  );
}
...
const paiementsAvant = paiements.filter((l) => l[0] !== "PAY-109").map((l) => (l[0] === "PAY-106" ? [...l.slice(0, 8), null, "À payer"] : l));
const livraisonsAvant = livraisons.map((l) => (l[0] === "PO-2026-036" ? [...l.slice(0, 4), d("2026-09-12"), null, "En mer"] : l));
...
const mapping: MappingChine = {
  onglets: [
    ...
    {
      onglet: "Livraisons",
      colonnes: { po: "N° PO", fournisseur: "Fournisseur", livraison_prevue: "Livraison prévue", livraison_reelle: "Livraison réelle", statut: "Statut" },
    },
  ],
};
...
console.log(`Fixture générée (${octets.length} octets) : ${versDonnees(actuel).onglets.map((o) => `${o.nom} (${o.lignes.length} lignes)`).join(", ")}`);
