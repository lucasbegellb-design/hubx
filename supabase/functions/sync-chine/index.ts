...
function lireClasseur(octets: Uint8Array): DonneesChine {
  ...
  const d = normaliserClasseur(
    wb.SheetNames.map((nom) => ({
      nom,
      lignes: XLSX.utils.sheet_to_json<unknown[]>(wb.Sheets[nom], {
        header: 1,
        raw: true,
        defval: null,
        blankrows: false,
...
servir(async (req) => {
  ...
  try {
    ...
    if (azure) {
      if (!source.share_url)
        throw new HttpError(422, "Aucun lien de partage : colle le lien du fichier dans Paramètres › Suivi Chine.");
    ...
    return json({
      ...
      change,
      snapshot,
      onglets: donnees.onglets.map((o) => ({ nom: o.nom, lignes: o.lignes.length })),
    });
  } catch (e) {
    const message =
      e instanceof HttpError
        ? e.message
        : "Synchronisation impossible (erreur inattendue). Nouvel essai au prochain passage.";
